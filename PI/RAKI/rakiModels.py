...
from MRI_Tools.PI.RAKI.utils.complexCNN import complexNet
from MRI_Tools.PI.RAKI.utils.shapeAnalysis import extractDatCNN, fillDatCNN
...
IRAKI_RECO_DEFAULT_ACS_NUM = 30#65


def rakiReco(kspc_zf, acs, R, layer_design,device='cpu'):
    '''
    ...
    src_kspc = torch.from_numpy(src_kspc).type(torch.complex64).to(device)#.requires_grad_()
    trg_kspc = torch.from_numpy(trg_kspc).type(torch.complex64).to(device)#.requires_grad_()
    # print(src_kspc.shape,trg_kspc.shape)
    
    net = complexNet(layer_design, R=R).to(device)    
    criterion = nn.MSELoss()
    
    optimizer = optim.Adam(net.parameters(), lr=RAKI_RECO_DEFAULT_LR) 
      
    for _ in range(RAKI_RECO_DEFAULT_EPOCHS):  #trange  
        optimizer.zero_grad()
    ...
    kspc_zf_input = torch.from_numpy(kspc_zf_input).type(torch.complex64).to(device)    
    # Estimate missing signals 
    net.eval()
    with torch.no_grad():
        kspc_pred = net(kspc_zf_input)['tot']
    kspc_pred = kspc_pred.cpu().detach().numpy()
...
def irakiReco(kspc_zf, acs, R, layer_design, grappa_reco, acs_flag, acs_start, acs_end,device='cpu'):
    '''
    ...
    for i in range(Niter):
        ...
        # training
        src_kspc = torch.from_numpy(src_kspc).type(torch.complex64).to(device)
        trg_kspc = torch.from_numpy(trg_kspc).type(torch.complex64).to(device)
        ...
        kspc_pred = net(kspc_zf_input.to(device))['tot']
        kspc_pred = kspc_pred.detach().cpu().numpy()
        kspc_pred = kspc_pred.transpose((0, 2, 3, 1))
        kspc_pred = np.squeeze(kspc_pred)
